propaneInterp2D = function(x, y, z = 1, image, xlim=NULL, ylim=NULL, pixcen='FITS', type = 'add', zero = FALSE) {
...
propaneBin2D = function(x, y, z = 1, image, xlim=NULL, ylim=NULL, pixcen='FITS', type = 'add', zero = FALSE) {
  if(length(x) != length(y)){
    stop('x and y must be same length')
  }
  if(length(x) != length(z) & length(z) != 1){
